...
contract PharmaChain_v3 is Ownable {
    struct Medicine {
        string name;
        string suppliers;
        uint256 batchNumber;
    ...
    // ✅ 查询药品信息
    function verifyMedicine(uint256 _batchNumber) public view returns (string memory, string memory, uint256, uint256, bool, string memory) {
        require(medicines[_batchNumber].batchNumber != 0, "Medicine not found");
        Medicine memory med = medicines[_batchNumber];
        return (med.name, med.suppliers, med.batchNumber, med.expiryDate, med.isApproved, med.qualityControlNotes);
    }
